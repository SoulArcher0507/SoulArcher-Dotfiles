...
if grep -q '^TerminalApplication=' "$CONFIG"; then
  sed -i 's|^TerminalApplication=.*|TerminalApplication=kitty|' "$CONFIG"
else
  # inserisce subito dopo la riga [General]
  sed -i '/^\[General\]/a TerminalApplication=kitty' "$CONFIG"
fi

echo "→ Impostazione TerminalApplication=kitty aggiunta/aggiornata in $CONFIG"
